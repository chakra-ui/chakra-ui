...
export interface RatingProps extends ThemingProps<"Switch"> {
  amount: number
  size: string
...
export const Rating = forwardRef<RatingProps, "input">(function Rating(
  props,
  ref,
) {
  const {
  ...
  const styles = useMultiStyleConfig("Rating", props)
  ...
  const { handleClick, handleHover, rating, hoveredRating } = useRating({
    hoveredValue: 0,
    value: 0,
  })
  ...
  return (
    <chakra.div
      className={cx("chakra-rating", props.className)}
      ...
      {[...Array(amount)].map((_, i) => {
        return (
          <StarIcon
            ...
            color={
              (i + 1 <= rating || hoveredRating > i) && i + 1 > rating
                ? `${fillColor}.400`
                : i + 1 <= rating
                ? fillColor
                : "white"
