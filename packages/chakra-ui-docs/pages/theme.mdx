...
[Smart Swatch](https://smart-swatch.netlify.app/), [Coolors](https://coolors.co/app) or [Palx](https://palx.jxnblk.com) to generate
these palette.
...
**Note:** This is `rem` units are relative to the root or html font size which
is `16px` by default. Chakra leaves this value unaltered.

Changing the root font size will make the `pixel` values above altered and
