...
___NOTE___: In the shortcut example `'100%'` is used instead of `1` because in the default Chakra UI theme, `theme.sizes[1] = 0.25rem`. This means that using a prop like `width={1}` will render a width of 4px and not `'100%'`
