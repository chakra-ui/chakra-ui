...
- `ModalTransition`: The transition provider, built with `react-spring`
...
yourself. Chakra exports `ModalTransition` that provides simple transitions.

> If use the `ModalTransition`, you might need to set `finalFocusRef` so it
> returns focus back, and also set `isOpen` to `true`.
      ...
      <ModalTransition isOpen={isOpen}>
        {styles => (
          <Modal finalFocusRef={btnRef} onClose={onClose} isOpen={true}>
            <ModalOverlay opacity={styles.opacity} zIndex={7} />
            <ModalContent
