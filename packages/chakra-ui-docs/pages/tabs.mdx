...
You can change the change the alignment of the `TabList` by passing `align`
...
Because TabList needs to know the order of the children, we use cloneElement to
pass state internally. If you want to know if a tab is active, you can wrap it,
and then inspect clone props passed in.
...
function Example() {
  const CoolTab = React.forwardRef((props, ref) => {
    ...
    return (
      <Tab ref={ref} isSelected={isSelected} {...props}>
        {isSelected ? "😎" : "😐"}
        {children}
      </Tab>
    );
...
| --------- | ------------------ | --------------------------------------------------------------------------- |
| Tab       | `role="tab"`       | Indicated that it's a tab                                                   |
