...
export function useVariantColorWarning(label, variantColor) {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (variantColorIsDefined) {
      ...
      // if variant color exists check if is a string or an object with 50 - 900 keys
      if (variantColorExists) {
        ...
        if (typeof colorObj !== "string") {
          ...
          const variantKeys = [
            "50",
            ...Array.from(new Array(9), (_, i) => `${i * 100 + 100}`),
          ];
          if (
            !colorObjKeys.length ||
            !colorObjKeys.every((elem) => variantKeys.includes(elem))
          ) {
            console.warn(
              `${label}: The variantColor passed exists in the theme object but is not valid. For a color to be a valid variantColor, it must be an object that has '50', '100' - '900' color values. Use a tool like: 
