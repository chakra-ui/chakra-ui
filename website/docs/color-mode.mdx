...
By default the current color mode is persisted in local storage. Unfortunately this can cause a flash between light and dark modes
during initial page load for apps that server side render. This flash is caused because local storage isn't accessible on the server.
The default storageManager can be swapped out to use cookies instead of local storage. Below is an example of using the 
