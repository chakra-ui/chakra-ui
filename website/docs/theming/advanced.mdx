...
All components are connected to a shared theming context so if you pass any
theming props (`size`, `variant` or `colorScheme`) to the root or parent
component, all sub-components can respond to that.
...
An example of this is Chakra's `Tabs`: `TabList`, and `Tab` componens.
...
At first, it might seem like a lot to setup component styles, however, we've
tested this API in a number of scenarios and it worked perfectly!

All Chakra components use this approach as well. If you check the
`chakra/components` folder, you'll see all component styles.
