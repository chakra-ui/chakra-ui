...
- Does not provide the logic to custom style them using `style props` with
  `theme` value or `themeKey`.

- Doesn't allow to use `ref`, since it doesn't get forwarded automatically. This
  may report a React error in the console about passing ref to a function
  component.
