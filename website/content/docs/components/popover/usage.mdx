...
- `Popover.Arrow`: A visual arrow that points to the reference (or trigger).
- `Popover.CloseTrigger`: A button to close the popover.
- `Popover.Anchor`: Used to wrap the position-reference element.
...
> Tip: If you want to render custom component instead of the `Popover.Trigger`,
> ensure you pass the `asChild` prop to the `Popover.Trigger` component the
> children passed to `Popover.Trigger` is focusable and has a forwarded `ref`.
...
> 🚨 Note: As with [Popover.Trigger](#basic-usage), if you use a custom
> component inside `Popover.Anchor` pass `asChild` prop and ensure that it uses
> Chakra's
...
<Popover.Root>
  ...
  <Popover.Positioner>
    <Popover.Content bg='tomato' color='white'>
      ...
      <Popover.CloseTrigger bg='purple.500' />
      <Popover.Body>
