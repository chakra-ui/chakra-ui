...
Set the `min` or `max` prop to set an upper and lower limit for the input. By
default, the input will restrict the value to stay within the specified range.

These props defaults to `MIN_SAFE_INTEGER` and `MAX_SAFE_INTEGER` respectively.
...
> The NumberInput will be set `invalid` to `true` internally when the value is
