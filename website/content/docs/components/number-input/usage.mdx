...
- `NumberInput.Root`: The wrapper that provides context and logic to the
  components.
- `NumberInput.Field`: The `input` field itself.
- `NumberInput.Stepper`: The wrapper for the input's stepper buttons.
- `NumberInput.IncrementStepper`: The button to increment the value of the
  input.
- `NumberInput.DecrementStepper`: The button to decrement the value of the
  input.
...
<NumberInput.Root size='sm' defaultValue={15} min={10}>
  <NumberInput.Field focusBorderColor='red.200' />
  <NumberInput.Stepper>
    <NumberInput.IncrementStepper
      bg='green.200'
