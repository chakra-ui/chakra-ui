...
<HStack gap={4}>
  {['sm', 'md', 'lg'].map((size) => (
    <Tag size={size} key={size} variant='solid' colorPalette='teal'>
      Teal
    </Tag>
  ))}
</HStack>
...
<HStack gap={4}>
  {['sm', 'md', 'lg'].map((size) => (
    <Tag size={size} key={size} variant='subtle' colorPalette='cyan'>
...
<HStack gap={4}>
  {['sm', 'md', 'lg'].map((size) => (
    <Tag size={size} key={size} variant='outline' colorPalette='blue'>
...
```jsx
<Tag size='lg' colorPalette='red' borderRadius='full'>
  <Avatar
