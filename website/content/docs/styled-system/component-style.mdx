...
part of Chakra UI? Let's use the following design spec for a Sample component:
...
```jsx live=false
const Sample = defineStyleConfig({
...
As with the Button component, we'll update the theme to include the new Sample
component style.
...
> Please note that name of a component you make must not be conflicted with any
> components built-in Chakra UI.

But in this case we'd have to **consume** these styles because the `Sample`
component is not a built-in component in Chakra UI.

### Consuming style config

Since the new Sample component is **not** part of Chakra UI we need to create a
...
function Sample(props) {
  ...
  const styles = useStyleConfig('Sample', { variant })
...
And lastly - the fun part - let's use our custom Sample component anywhere in
our app:
