...
`Text` and `Heading` elements have a prop called `isBaselineCropped`. This prop
...
In order to use `isBaselineCropped`, you must measure your font manually and
enter those measurements into the `baselineCrop` key in your theme.
...
<Text isBaselineCropped>Mark Dalgleish: Meme Lord</Text>
<Box height="10px" width="100%" bg="green.800" />
```
...
| ------------------- | --------- | ------- | --------------------------------- |
| `isBaselineCropped` | `boolean` |         | See `Baseline Crop` section above |
