...
## Theme Typings <Badge fontSize="sm" colorScheme="teal" letterSpacing="wider">v1.2.0</Badge>
...
for your application theme. If you want to learn how to scale your custom theme
you can
...
> You need to run this command after every `npm install` or update to the theme
> files, because it updates your node_modules in place
...
The autocomplete for theme token only works in _Visual Studio Code_ for now. We
keep an eye on the TypeScript support of Jetbrains IDEs, but I we have to wait
until it is improved.

Please note that the CLI cannot read non-JavaScript/TypeScript files. If you are
importing additional e.g. `.css`, `.woff` or `.svg` files, you need to move
those imports out of the theme files.
