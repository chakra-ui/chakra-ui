...
The `RangeSlider` is a slider with two or more thumbs that each set a value in a
group of related values. A common use-case of this component is a price range
...
`RangeSliderThumb` composes [Box](/docs/layout/box) so you can pass all `Box`
props to change its style.

### RangeSliderFilledTrack Props

`RangeSliderFilledTrack` composes [Box](/docs/layout/box) so you can pass all
`Box` props to change its style.

### RangeSliderTrack Props

`RangeSliderTrack` composes [Box](/docs/layout/box) so you can pass all `Box`
props to change its style.
