...
Chakra UI v1.0 is focused on improving the ideas and concepts from 0.x to make it 
...
We've updated the API of some components to fix bugs, improve usability,
...
- Deprecated the `isFullWidth` prop. `Checkbox` now takes up the width of it's
  parent by default.
...
- Deprecated the `isFullWidth` prop. The `Radio` takes up the width of it's
  parent by default.
...
- Added support for `onCloseComplete` prop, which is a callback function that is invoked when the toast closes.
...
- Removed the `config` prop, in favor of adding global styles to `theme.styles.global`.
- Added global style for `focus-visible` package. Once installed, it removes focus
