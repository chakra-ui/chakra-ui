...
1. Add the `ColorModeScript` to your application, and set the initial color mode
   your application should start with to either `light`, `dark` or `system`. It
   is `light` by default.

2. Update your theme config to determine how Chakra should manage color mode
   updates.
...
For Next.js, you need to add the script the the `_app.js` file.
...
For Next.js, you need to add the script the the `gatsby-ssr.js` file.
