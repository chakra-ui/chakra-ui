...
```jsx live=false ln={4,10-12} viewlines
// pages/_app.js
...
function LangSwitcher() {
  ...
  const onClick = async () => {
    ...
    // force a reload if it work correctly.
    reload()
  }
...
`direction` key to the theme. Then, add the custom theme to the
`ChakraProvider`.
...
| Style Props                                     | Replace with                                    | Description                               |
| ----------------------------------------------- | ----------------------------------------------- | ----------------------------------------- |
...
## RTL Implementation References

- [Blog Post - Setup an internationalization (i18n) routing web app with Nextjs and Chakra UI](https://dev.to/ugbechike/setup-an-internationalization-i18n-routing-web-app-with-nextjs-and-chakra-ui-39f1)
