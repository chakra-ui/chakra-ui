...
Wrap is a layout component that adds a defined space between its children. It 'wraps' its children automatically if there isn' not enough space to fit any child.
...
```
- **WrapItem**: WrapItem composes the `Box` component. It renders the HTML `<li>` tag in the DOM.
...
| --------- | ------------------------------ | -------- | ------------------------------------------------------ |
| spacing   | `Responsive(string or number)` | `0.5rem` | The space between each child (even if it wraps).        |
