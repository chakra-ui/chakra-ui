---
title: Dark Mode
description: How to use dark mode in Chakra UI applications
---
...
To trigger the dark mode version of components, set the `dark` className on any
parent element, or the root element of your application. This is what
`next-themes` does under the hood.
...
To reduce the amount of code you need to write, prefer to use semantic tokens to
style components for dark mode. This ensures the light and dark mode styles are
...
To force dark mode, you can set the `dark` className on any parent element, or
