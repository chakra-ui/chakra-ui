...
description: Using the chakra factory to create supercharged components
---

## Overview

Chakra factory serves as a way to create supercharged JSX element from any
component to enable them receive JSX style props.
...
Style props are just CSS properties that you can pass to your components as
props. With the JSX factory, you can use `chakra.<element>` syntax to create
supercharged JSX elements that support style props.
...
key requirement is that the component **must** accepts `className` as props.
...
The `chakra` function, styles or recipes can be attached to the component.
