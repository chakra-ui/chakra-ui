...
Let's say you want to change the font weight of a text on large screens, you can
do it like this:
...
or you can use the prop based modifier
...
Let's say we want to apply styles between the `md` and `xl` breakpoints, we use
the `mdToXl` property:
...
To target a single breakpoint, use the `Only` notation. Let's say we want to
apply styles only in the `lg` breakpoint, we use the `lgOnly` property:
...
Let's say we want to hide an element from the `md` breakpoint, we use the
`hideFrom` utility:
...
Let's say we want to hide an element below the `md` breakpoint, we use the
`hideBelow` utility:
