...
For example, to change the background color of a button when it's hovered,
here's to do it:
...
For example, to change the background color of an element is focused and
hovered. Here's how to do it:

```jsx
<Box bg={{ base: "red.500", _hover: { _focus: "red.700" } }}>Hocus me</Box>
```
...
```jsx
<chakra.button _hover={{ bg: "red.700" }} _active={{ bg: "red.900" }}>
  Hover me > Hover me
</chakra.button>
```
...
<Box as="ul">
  {items.map((item) => (
    <Box as="li" key={item} _first={{ color: "red.500" }}>
      {item}
    </Box>
  ))}
</Box>
...
requested the system use a light or dark color theme.
...
To style an element based on its sibling element's state or attribute, you can
add the `peer` class to the sibling element, and use any of the `_peer*`
modifiers on the target element.
...
[here](/docs/customization/conditions).
