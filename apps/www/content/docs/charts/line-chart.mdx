...
To highlight a specific data point, use the reference components from
`recharts`.
...
To set the domain (upper and lower bounds) of the value axis, use the `domain`
prop in the `YAxis` component.
...
Here are the different types of line charts you can create:
