...
> We use the `Cell` component from `recharts` to color the bars.
...
Use the `layout` prop to the `Legend` component from `recharts` to configure the
...
To make reference to a specific value on the chart, use the `ReferenceLine`
