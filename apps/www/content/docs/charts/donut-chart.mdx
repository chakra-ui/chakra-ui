...
To display a point label on the chart, you can use the `PointLabel` component
from `recharts`.
...
To add some space between the segments, you can use the `paddingAngle` prop.

> **Pro Tip:** You can also use the `cornerRadius` prop to round the corners of
> the segments.
