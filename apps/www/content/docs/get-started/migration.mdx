...
Remove the unused packages: `@emotion/styled` and `framer-motion`. Next, install
the CLI snippets.
...
Also upgrade `@chakra-ui/react` and `@emotion/react` to the latest version:

```bash
npm i @chakra-ui/react @emotion/react
```

### Move Custom Theme to Separate File (if you have one)

If your custom theme isn’t in a separate file, move it to a dedicated `theme.js`
or `theme.ts` file.

Next, usse `createSystem` and `defaultConfig` to configure your theme.
...
In version 3, the `ChakraProvider` has been renamed to `Provider`. You'll need
to update your imports and component usage in your `app.js`, `app.tsx`, or
equivalent file.

Update the wrapping component to `Provider` and pass the `system` you created
earlier.

Also rename the `theme` prop to `value` to match the new system-based theming
approach.
...
> Note that `Provider` is imported from chakra snippets.
...
- **Snippets:** Introduced snippets that provide pre-built components for faster
  UI development. Use the `@chakra-ui/cli` to
  [add snippets](/docs/get-started/cli#chakra-snippet) to your project.
