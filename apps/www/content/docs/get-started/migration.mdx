...
Install updated versions of the packages: `@sh3yk0-ui/react` and
`@emotion/react`.

```bash
npm install @sh3yk0-ui/react@latest @emotion/react@latest
```
...
Update the ChakraProvider import from `@sh3yk0-ui/react` to the one from the
...
import { defaultSystem } from "@sh3yk0-ui/react"
...
We've removed the `@sh3yk0-ui/next-js` package in favor of using the `asChild`
...
```tsx {3}
import { Button as ChakraButton, forwardRef } from "@sh3yk0-ui/react"
...
```tsx {2, 4}
import { Button as ChakraButton } from "@sh3yk0-ui/react"
...
Removed `@sh3yk0-ui/icons` package. Prefer to use `lucide-react` or
`react-icons` instead.
