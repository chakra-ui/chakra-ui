...
## Defining Tokens in Separate Files

As your token set grows, you can move tokens into their own files and import
them into your system config. Each token must already be authored in the
`{ value: ... }` shape in that file.

```ts title="theme/colors.ts"
export const colors = {
  brand: {
    500: { value: "#0066cc" },
  },
...
`defineTokens` is a type helper. It's an identity function that returns its
argument unchanged, so it only provides autocomplete and type-safety — it does
**not** transform or reshape values at runtime. Your tokens must already be
authored in the `{ value: ... }` shape; `defineTokens({...})` won't wrap raw
values for you.
