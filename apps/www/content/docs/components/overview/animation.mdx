...
When a disclosure component (popover, dialog), the `data-state` attribute is set
to `open`. This maps to `data-state=open` and can be styled with `_open` pseudo
prop.
...
Here's an example of creating a fade-in animation using keyframes:
...
Here's an example of creating a fade-out animation using keyframes:
