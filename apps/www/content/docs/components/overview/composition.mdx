...
Chakra UI components offers two powerful props for customization: `as` and
`asChild` prop.

## The `as` Prop

Used to change the underlying HTML element a React component renders. It
...
The `asChild` Prop: Used to compose a component's functionality onto its child
element. This approach, inspired by
...
There are a few rules of thumb to consider when using the `as` and `asChild`
props to avoid common pitfalls.
