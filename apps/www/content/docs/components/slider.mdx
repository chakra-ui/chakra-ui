...
The Slider component follows the
...
- `PageUp` - Increases the slider value by a larger step (typically 10%)
- `PageDown` - Decreases the slider value by a larger step (typically 10%)

#### Vertical Orientation

All keyboard interactions are the same, with `ArrowUp` increasing and
`ArrowDown` decreasing the value.

### ARIA Attributes

- The slider thumb has `role="slider"`
- The thumb has `aria-valuemin`, `aria-valuemax`, and `aria-valuenow` attributes
- The thumb uses `aria-orientation` set to `horizontal` or `vertical`
- When a label is present, the slider uses `aria-labelledby` to reference it
- For multi-thumb sliders, each thumb is properly labeled and has distinct ARIA
  attributes
