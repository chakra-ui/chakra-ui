...
The `Slider.Thumbs` component is a shortcut for the `Slider.Thumb` and
`Slider.HiddenInput` components. It renders a thumb for each value.
...
The `Slider.Marks` component is a shortcut for the `Slider.MarkerGroup` and
