...
Don't mount a popover per button. Put `value` on each `Trigger` under one
`Root`. Click another trigger and the same popover moves to it.

Track the active trigger with `triggerValue` and `onTriggerValueChange` on
`Root`. If the target is not a button, use `positioning.getAnchorRect`.

<ExampleTabs name="popover-with-multiple-triggers" />
...
<PropTable component="Popover" part="Trigger" />
