...
- Use the `disabled` prop instead of `aria-disabled` when possible for proper
  keyboard handling
- For loading states, the button is disabled and marked with `data-loading`; add
  `aria-busy="true"` manually if needed for your use case

### Button as Link

When using `asChild` to render a button as a link, ensure proper keyboard
navigation is maintained. Links should navigate, buttons should perform actions.
