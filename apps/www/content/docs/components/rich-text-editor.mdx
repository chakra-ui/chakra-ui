...
You only need to install extensions for the controls you use. Controls can be
freely added or removed based on your editor configuration.
...
an image workflow that matches your product needs—whether that’s embedding URLs,
uploading files, or integrating with a custom media service. To enable image
support, you must also install and register Tiptap’s Image extension.
...
Tiptap’s Emoji extension. Emojis can be triggered by typing `:` or using common
