...
The `NumberInput` component also provides a set of shortcuts for common use
cases.

### NumberInputControl

The `NumberInput.Control` renders the `NumberInput.IncrementTrigger` and
...
This might be more concise, if you don't need to customize the increment or
decrement triggers:
...
> To disable the spin functionality, pass the `spinOnPress` prop to the
> `NumberInput.Root` component and set it to false.
