...
Don't mount a hover card per link. Put `value` on each `Trigger` under one
`Root`. Hover another trigger and the same card moves to it.

<ExampleTabs name="hover-card-with-multiple-triggers" />
...
<PropTable component="HoverCard" part="Trigger" />
