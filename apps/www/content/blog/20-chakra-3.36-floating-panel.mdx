...
- **Ark UI 5.37.2**: Updated our headless foundation. Splitter now accepts
  CSS-unit sizes (`px`, `rem`, `vw`) and a per-panel `resizeBehavior`, floating
  components expose `data-side` for placement-aware styling, and Combobox, Date
  Picker, and Tabs pick up a batch of fixes.
