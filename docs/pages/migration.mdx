...
stressful for you to customize them.

1. Run `npx chakra init --theme`, and we'll clone the theme to your project.
   This creates a `chakra` folder with the default theme inside.

```sh
npx chakra init --theme
```

2. Update the `ThemeProvider` in your application's root by passing the `theme`
   prop.
...
- Added the `labelSpacing` prop to customize the spacing between the checkbox
  and label text
...
package which will automatically configure `InitializeColorMode` along with
`ChakraProvider`.
