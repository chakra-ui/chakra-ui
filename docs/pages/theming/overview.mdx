...
fully customizable, theme-aware components directly from your theme.

## Style modifiers

> Modifiers are attributes that change the visual appearance of a component

After reviewing several components across the web, we noticed that most reusable
components have the following style modifiers:
...
In addition to these, most components have **base or default styles** regardless
of the modifiers. We call that the `baseStyle`.
...
First thing we notice is that the badge as some common or default styles across
all badge variants. Here's how we'll model that:
