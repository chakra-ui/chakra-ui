...
## [2.10.10](./#2.10.10) - 2026-06-04

### Fixed

- **System**: Fix infinite loop in `objectToArrayNotation` when a responsive
  object has trailing `null` values.
- **Toast**: Add `aria-hidden` attribute to the toast container when no toasts
  are present.

## [2.10.9](./#2.10.9) - 2025-05-21

### Fixed

- **System**: Fix issue in React 19 where SSR rendered components freeze in dev
  mode when non-memoized react components are passed to `useStyleConfig`

## [2.10.8](./#2.10.8) - 2025-05-14
...
## [2.10.7](./#2.10.7) - 2025-03-14

### Fixed

- **CLI**: Bump `bundle-n-require` to fix vulnerability warning.
- **Checkbox**
...
- Use `useEmotionCache` instead of `withEmotionCache` to prevent style loss
  during rehydration across multiple Suspense boundaries.

## [2.10.5](./#2.10.5) - 2025-01-18
...
- **Popover**:
  ...
  - Fix issue where popover doesn't use the correct zIndex token.
  - Fix issue where popover not closing on outside click when it's not focused
...
- Removed export of `keyframes` was not exported from `@chakra-ui/react`. Please
  import it from `@emotion/react` instead.

- Removed export of `useId`. Please import it from `react` instead.

## [2.9.5](./#2.9.5) - 2024-10-04

### Fixed

- **Next.js:** Fix issue where emitted ESM doesn't resolve correctly due to
...
## [2.9.4](./#2.9.4) - 2024-10-03

### Fixed

- **Extend Theme:** Fix issue where extend theme utilities like
  `withDefaultColorScheme` were not exported correctly.

## [2.9.3](./#2.9.3) - 2024-10-02

### Fixed

This release fixes issues with the build process caused by regressions in
`2.9.1` and `2.9.2`. It ensures components and types are exported correctly.

## [2.9.0](./#2.9.0) - 2024-09-30

### Fixed

- **Menu:** Fix hard-coded z-index for Menu in favor of one defined from the
  theme

- **Storybook:** Fix issue in storybook addon where theme could be undefined.

- **Avatar**: Fix problem with leading and trailing spaces when getting
  initials.

- **ColorMode**: Include "nonce" from "EmotionCache" inside style tag to resolve
  Content Security Policy error

- **Hooks**: Fix issue where `useOutsideClick` doesn't work in shadow DOM
  environments
...
- **Styled System**:
  ...
  - Add CSS `accentColor` property to styled-system
  - Add support for custom theme conditions or pseudo props via
    ...
    **For TypeScript users**, you need to use the Chakra CLI to generate the
    types for your custom conditions.

    ```sh
    pnpm chakra-cli tokens src/theme/index.ts
    ```

  - Add support for `_open` and `_closed` pseudo props for styling their
...
- **Toast**: Export `toastStore` from `toast` component

- **Checkbox**: Add for attribute to Radio and Checkbox label if id is available
  for the input

### Changed

- **Button**: Revert breaking change by re-allowing disabled prop on button
