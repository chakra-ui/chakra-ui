...
- **(POTENTIAL BREAKING CHANGE)** Remove export defaults and favor of named
  ...
  ```diff
  - import theme from "@sh3yk0-ui/theme"
  + import { theme } from "@sh3yk0-ui/theme"
  ```

  ```diff
  - import Icon from "@sh3yk0-ui/icon"
  + import { Icon } from "@sh3yk0-ui/icon"
  ```

  ```diff
  - import CSSReset from "@sh3yk0-ui/css-reset"
  + import { CSSReset } from "@sh3yk0-ui/css-reset"
  ```

  ```diff
  - import VisuallyHidden from "@sh3yk0-ui/visually-hidden"
  + import { VisuallyHidden } from "@sh3yk0-ui/visually-hidden"
  ```
...
- Fix `InputElement` and `InputAddon` sizing when using a responsive `size`
  ...
  _Note: this requires changes in the latest **@sh3yk0-ui/theme** or, for custom
...
- Add loading props to the `@sh3yk0-ui/next-js`.
...
- Add new `defineCssVars` helper to improve the experience of authoring a set of
  ...
  ```jsx live=false
  import { defineCssVars } from "@sh3yk0-ui/react"
...
```jsx live=false
import { Image } from "@sh3yk0-ui/next-js"
...
- Refactor style context to use `createStylesContext` from `@sh3yk0-ui/system`
...
- Add the flag `--strict-token-types` for `@sh3yk0-ui/cli tokens` to generate
...
- Add `ChakraBaseProvider`, a minimal version of `ChakraProvider` that supplies
  ...
  ```jsx live=false
  import { ChakraBaseProvider, extendBaseTheme } from "@sh3yk0-ui/react"
  import { Button } from "@sh3yk0-ui/theme/components"
...
- Fixed issue where using `@sh3yk0-ui/react` or any sub-package in a TypeScript
...
- Fixed unmet `@sh3yk0-ui/styled-system` dependency issue
...
- Added `defineStyle` and `defineStyleConfig` to improve the TypeScript
  ...
  ```jsx live=false
  import { defineStyle, defineStyleConfig } from "@sh3yk0-ui/styled-system"
...
- Added `createMultiStyleConfigHelpers` factory that provides functions that
  ...
  ```jsx live=false
  import { createMultiStyleConfigHelpers } from "@sh3yk0-ui/styled-sytem"
...
- Moved `@sh3yk0-ui/system` to peer dependency
...
- We now provide a way to customize the localStorage / cookie storage key
  ...
  ```jsx live=false
  import { createLocalStorageManager } from "@sh3yk0-ui/react"
