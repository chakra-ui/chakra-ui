...
## [3.37.0](./#3.37.0) - 2026-08-28

### Added

- **DateInput [NEW]**: Add a segmented date field for typing dates without a
  calendar.
...
- **Accordion / Collapsible / Dialog / Drawer / TreeView**: Add `hideMode` to
  choose how content that stays mounted is hidden when closed. The default,
...
- **Codemod**: `upgrade --dry` now lists the files that would change and, per
  file, the components and props it touches (e.g. `Button, color palette`).
  Large projects get the full breakdown in `chakra-codemod-report.md`, smaller
...
- **Presence**: Add `onEnterComplete` for when enter animations finish,
  mirroring the existing `onExitComplete`. Affects ColorPicker, Combobox,
  DatePicker, Dialog, Drawer, FloatingPanel, HoverCard, Menu, Popover, Select,
  Tooltip, and Tour.
...
- **Codemod**
  - Fix `.ts` files being parsed as `tsx`, so type assertions and generic arrows
    no longer throw and get skipped.
  - Fix the unused-package check scanning `node_modules`, which flagged
    `framer-motion` and `@emotion/styled` as used when only a dependency
    imported them.
  - Fix the `color-mode` transform installing a snippet during a dry run.
  - Fix the `steps` transform adding a stray `Steps` import to files that don't
    use a stepper.
...
- **DateInput**
  ...
  - Fix `CalendarDate` and `CalendarDateTime` values shifting by your local UTC
    offset when you pass a custom `formatter` without a `timeZone`.
...
- **Dialog / Drawer**
  ...
  - Fix the page still scrolling behind an open overlay on layouts where
    `<html>` is the scroll container. The scroll lock targeted `<body>`, so
    nothing was locked.

- **Dialog / Drawer / Menu / Popover**: Fix `Escape` being ignored right after
  an overlay opens. Handlers registered a frame late, so the overlay was painted
...
- **Dialog / Drawer / Popover**: Fix a closing overlay pulling focus back from
  ...
  right after closing the first. Closing a nested overlay no longer throws when
  the outer container has no connected focusable element, and the focus ring now
...
- **Preflight**: Fix semantic `i` and `em` elements not rendering in italics.
...
- **System**: Fix Next.js 15 production builds failing to compile with
...
- **Tag / ActionBar / Dialog / Drawer**: Fix `Tag.CloseTrigger`,
  `ActionBar.SelectionTrigger`, `Dialog.ActionTrigger`, and
  `Drawer.ActionTrigger` missing `type="button"`, which submitted the
  surrounding `<form>` on click.
...
- **TreeView**: Fix `--tree-indentation: 0px` not fully removing nested
  indentation.

### Changed

- **Codemod**: Run `upgrade` transforms in-process instead of spawning a Node
  process per transform, taking a full run from around 30s to a few seconds.

- **TreeView**: `--tree-indentation` is now the full per-level indent
  (`indent-size + half icon-size`). Custom non-zero values no longer get an
  extra half-icon offset on top, and the internal `--tree-icon-offset` variable
  has been removed from the recipe.

## [3.36.1](./#3.36.1) - 2026-07-19
...
- **CLI**: Normalize resolved tsconfig paths to native separators on Windows.
...
- **LinkOverlay**: Fix issue where `LinkOverlay` dropped the `rel` attribute
  instead of forwarding it to the rendered anchor.

- **RadioCard / CheckboxCard**: Fix issue where the checked ring (outline
  variant) gets clipped when a parent has `overflow: hidden|auto|scroll`. The
  ring is now drawn with an inset shadow instead of an outer shadow.

- **System**: Fix error when merging recipes (e.g. composing a recipe-based
  component through the `chakra` factory). Recipe merging now normalizes
  compiled and raw configs before combining them, and no longer throws or
  mutates the source configs.
...
- **CLI**
  - Migrate from deprecated `tsconfck` to `get-tsconfig`. Enables installation
    next to TypeScript version 6 and up.
  - Replace `node-fetch` with the native `fetch` API, using undici's
    `ProxyAgent` for `HTTPS_PROXY` support.

- **Codemod**: Require Node.js >=22 (Node 18 and 20 are end-of-life).

- **FloatingPanel**: Set a default `minSize` of `{ width: 240, height: 100 }` on
  `FloatingPanel.Root` to prevent the panel from being resized to zero. Pass
  your own `minSize` to override it.

- **OverlayManager**: Expose `has()` on the return value of `createOverlay`.

- **Panda Preset**: Sync theme from `@chakra-ui/react`: register the missing
  `floatingPanel` slot recipe and update the date picker range-selection styles.

- **System**: Improve render performance of recipe components (`Button`,
  `Badge`, `Skeleton`, etc.) in large lists and tables.
...
## [3.35.0](./#3.35.0) - 2026-04-22

### Added

- **CLI**: Add new CLI commands mirroring the MCP server tools:
  ...
  - `chakra theme` — Summary table of theme categories (use `--json` for full
    data or `--filter <category>` for one section)
  - `chakra docs <query>` — Search the Chakra UI documentation

- **Dialog / Popover / HoverCard**: Add support for multiple triggers sharing
  one dialog instance.

- **Field**: Add `Field.Item` component and `target` prop on `Field.Root` for
  multi-control fields.

- **FileUpload**: Reject duplicate files with `FILE_EXISTS` error code.

- **Listbox**: Add `keyboardPriority` for Home/End and arrows; add
  `highlightFirst`, `highlightLast`, `highlightNext`, and `highlightPrevious`
  methods.

- **Menu**: Set `aria-expanded` when closed; support multiple triggers.
...
- **PinInput**: Add `enterKeyHint`, `autoSubmit`, and `sanitizeValue` props.

- **Popover**: Add support for `translations`, `finalFocusEl`, and
  `restoreFocus` props.

- **TagsInput**: Add `allowDuplicates` and `sanitizeValue` props, plus
  `enterKeyHint` on mobile.

### Fixed

- **Carousel**: Fix controlled carousel inside dialog, navigation in transformed
  containers, scroll drift, and page sync with indicators.

- **ColorPicker**: Fix vertical slider orientation on pointer updates.

- **Combobox**: VoiceOver now announces highlighted options on Apple devices via
  a live region.

- **DatePicker**: Fix calendar popup clipping when constrained by available
  height.

- **Dialog / Drawer**: Fix panel sometimes showing behind the dimmed overlay
  when opening and closing quickly or with certain global z-index styles on the
  page.
...
- **Menu**: Fix glitchy submenu "diagonal" hover flash.

- **PinInput**: Fix deletion and focus behavior, and Home/End navigation.

- **System**
  - Fix `isCssUnit` utility to reject malformed values like `1a5rem` and
    `1-5rem` by properly escaping the decimal point in the length regex.
  - Fix array shorthand for fonts, shadows, gradients, animations, and easings
    (no longer mistaken for responsive arrays).
  - Fix responsive array values in `globalCss` selector rules (for example `#id`
    or `.class`) being serialized incorrectly instead of generating responsive
    breakpoint styles.

## [3.34.0](./#3.34.0) - 2026-03-03
...
- **Charts**: Upgrade recharts to 3.7.0 and prefer `responsive` prop over
  `ResponsiveContainer` when using `Chart.Root`. The `responsive` prop avoids
  React 19 compatibility issues that `ResponsiveContainer` can trigger.

## [3.33.0](./#3.33.0) - 2026-02-11

### Added

- **Scroll Area**: Add overflow CSS variables for scroll fade effects
...
- **CodeBlock**: Fix overlay and floating elements scrolling out of view when
  horizontally scrolling long code lines

- **Color Picker**: Fix color not updating when selecting black shades in
  controlled mode

- **Dialog/Popover**: Fix issue where closing nested dialogs/popovers would
  incorrectly close parent layers

- **Menu**: Fix glitchy submenu behavior when hovering between trigger items
  quickly

- **Number Input**: Fix cursor positioning issues after clicking label or
  scrubbing
...
- **Textarea**: Fix change event not being emitted after clearing controlled
  textarea
...
- **cva**: Normalize base styles to prevent shorthand properties from
  overwriting variant styles

- **Menu**: Fix content background not rendering by using full token path for
  CSS variable
...
- **Badge, Button, Checkbox, Tag**: Update outline variants to use
...
- **Tabs**: Fixed indicator showing before rect resolved; fixed position not
  updating when inactive tabs resize

### Changed

- **useListCollection**: `initialItems` now accepts `readonly` arrays

## [3.29.0](./#3.29.0) - 2025-11-05

### Added

- **Carousel [New]**: Add new `Carousel` component for creating slideshows and
...
- **Combobox**: Refactor recipe for smarter padding management to prevent input
  text from overflowing onto triggers

## [3.28.1](./#3.28.1) - 2025-11-02
...
- **Styled System**
  - Fix Shadow DOM selector handling in `globalCss` for `:host`,
    `:host-context()`, and `::slotted()` pseudo-classes
  - Improve performance with optimized token cloning (75x faster), enhanced
    memoization with LRU cache (up to 585x faster), and efficient object
    allocation

### Changed

- **SegmentGroup**: Refactor to use CSS variables for styling indicator
...
- **mergeRefs**: Add React version detection to properly handle cleanup
  functions in React 19+ while avoiding warnings in React 18
...
- **CodeBlock**:
  - Fix issue where Line numbers display incorrectly when `meta.wordWrap` is
    true
...
- **Tooltip**: Change default delay values to improve accessibility.
...
- **CodeBlock**: Add configurable theme support and sync loading for Shiki
  adapter
...
## [3.24.1](./#3.24.1) - 2025-08-05

### Changed

- **Prose**: Provide a way to exclude components from prose styling by using
  `not-prose` class

### Fixed

- **System**: Fix issue where `fullscreen` condition returns the wrong css
  selector

## [3.24.0](./#3.24.0) - 2025-07-31

### Added

- **CodeBlock**: Add new codeblock component with support for highlight.js and
  shiki.
