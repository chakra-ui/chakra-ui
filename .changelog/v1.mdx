...
- Modify theme types to make it possible to customize token types via TypeScript
  ...
  > types (or custom output from **@sh3yk0-ui/cli**)

  ```ts
  import { BaseThemeTypings } from "@sh3yk0-ui/styled-system";

  type DefaultSizes = 'small' | 'medium' | 'large';

  declare module "@sh3yk0-ui/styled-system" {
...
- Introducing **semantic tokens**
  ...
  ```tsx live=false
  import { ChakraProvider, extendTheme } from "@sh3yk0-ui/react"
...
- Add entrypoints to the different parts of the theme (colors, fonts,
  ...
  import colors from "@sh3yk0-ui/theme/foundations/colors"
  ```
  ...
  | Part        | Entrypoint                                  |
  | ----------- | ------------------------------------------- |
  | components  | `"@sh3yk0-ui/theme/components"`             |
  | foundations | `"@sh3yk0-ui/theme/foundations"`            |
  | colors      | `"@sh3yk0-ui/theme/foundations/colors"`     |
  | sizes       | `"@sh3yk0-ui/theme/foundations/sizes"`      |
  | spacing     | `"@sh3yk0-ui/theme/foundations/spacing"`    |
  | typography  | `"@sh3yk0-ui/theme/foundations/typography"` |
  | radius      | `"@sh3yk0-ui/theme/foundations/radius"`     |
  | shadows     | `"@sh3yk0-ui/theme/foundations/shadows"`    |
  | transition  | `"@sh3yk0-ui/theme/foundations/transition"` |
  | zIndex      | `"@sh3yk0-ui/theme/foundations/z-index"`    |
  | blur        | `"@sh3yk0-ui/theme/foundations/blur"`       |
  | borders     | `"@sh3yk0-ui/theme/foundations/borders"`    |
...
- Add helper function `flatten`
  ...
  ```ts
  import { flatten } from "@sh3yk0-ui/utils"
...
- Added dependency to @sh3yk0-ui/icons
...
- Use the feature flag `--strict-component-types` for `@sh3yk0-ui/cli tokens` to
...
- The official Storybook Addon for Chakra UI.
  ...
  ```sh
  yarn add -D @sh3yk0-ui/storybook-addon
  ```

  ```sh
  npm i -D @sh3yk0-ui/storybook-addon
  ```
  ...
  ```js live=false
  module.exports = {
    addons: ["@sh3yk0-ui/storybook-addon"],
  }
  ```
...
- Update storybook url configuration for `@sh3yk0-ui/react` to
...
- Use @sh3yk0-ui/react-env to determine the correct body
...
- Fix: always use fromEntries polyfill from @sh3yk0-ui/utils
...
- Added missing `@sh3yk0-ui/react-utils` import
...
- Add new helpers to the `theme-tools` package to make the process of creating
  ...
  ```jsx
  import { calc, cssVar } from "@sh3yk0-ui/theme-tools"
  ...
  ```jsx
  import { anatomy }  from "@sh3yk0-ui/theme-tools"
  import type { PartsStyle } from "@sh3yk0-ui/theme-tools"
...
- Move ChakraProvider to a separate package `@sh3yk0-ui/provider`
...
```js
import { styled } from '@sh3yk0-ui/react'
...
```ts
import { useTheme } from "@sh3yk0-ui/react"
